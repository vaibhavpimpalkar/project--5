...
const createProduct = async function (req, res) {
    ...
    if (!validation.checkInputsPresent(price)) {
        return res.status(400).send({ status: false, msg: "please enter price" })
    }
   
    if (!validation.isValidNumber(price) || !validation.isValidPrice(price)) {
...
const getProductsById = async function (req , res){
   ...
   if(!validation.validateId(productId))return res.status(400).send({ status: true, message: "Invalid productId" })

    //database call for find product from product model
   const findProductsData = await userModel.findById({ _id: productId });
